 ...
 * is "en_US".
 */
export default function simpleDateFormat(pattern, locale) {
  this.locale =
    locale &&
    (locale == 'es_US' ||
      locale == 'en_CA' ||
  ...
  this.format = function (unformattedDate) {
    ...
    if (!(unformattedDate instanceof Date)) {
      var unformattedDateParts = unformattedDate.split('T')[0].split('-'),
        unformattedDateTimeParts =
          unformattedDate.split('T').length > 1
            ? unformattedDate.split('T')[1].split('.')[0].split('Z')[0].split('-')[0].split(':')
            : ['00', '00', '00']
      unformattedDate = new Date(
        unformattedDateParts[0],
        unformattedDateParts[1] - 1,
        unformattedDateParts[2],
        unformattedDateTimeParts[0],
        unformattedDateTimeParts[1],
        unformattedDateTimeParts[2]
      )
    }
    ...
    var formattedDate,
      ...
      patternReplace = function (patternPart) {
        var patternPartFormatted = patternPart
            .replace(/yy+(?=y)/g, 'yy')
            .replace(/MMM+(?=M)/g, 'MMM')
            .replace(/d+(?=d)/g, 'd')
            .replace(/EEE+(?=E)/g, 'EEE')
            ...
            .replace(/h+(?=h)/g, 'h')
            .replace(/m+(?=m)/g, 'm'),
          formattedPart = patternPartFormatted
            .replace(/yyy/g, dateParts.year)
          ...
          adjustTimePattern = function (timeParts, timePatternPart, operator) {
            for (var i = 1; i < timeParts.length; i++) {
              if (!isNaN(timeParts[i].substring(0, 1))) {
                ...
                if (
                  (timePatternPart == 'h' && timePartResult == 0) ||
                  (timePatternPart == 'hh' && timePartResult == '00')
                ) {
                  timePartResult = '12'
                }
        ...
        formattedPart = formattedPart
          .replace(
            /hh/g,
            dateParts.hour12 < 12 && dateParts.hour12.indexOf && dateParts.hour12.indexOf('0') != 0
              ? '0' + dateParts.hour12
              : dateParts.hour12
          )
        ...
        formattedPart = formattedPart
          .replace(/mm/g, dateParts.minutes)
        ...
        var formattedMonthShorthand3 = formattedMonthNames[Number(dateParts.month) - 1].substring(
          0,
          3
        )
        if (locale == 'fr_CA') {
        ...
        formattedPart = formattedPart
          .replace(/EEEE/g, formattedDayNames[dateParts.day])
        ...
        if (locale != 'es_US' && locale != 'fr_CA') {
          formattedPart = formattedPart.replace(/mar/g, 'Mar').replace(/may/g, 'May')
        }
